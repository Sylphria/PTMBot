...
function answer($text){
	...
	while(true){
		...
		if(preg_match($regex ,$text)){	
			$hint = "คู่มือการขอหรือแก้ไขรหัส POLIS/PTM
		...
		$regex = '/(ช่วยราชการ)/i';
		if(preg_match($regex ,$text)){	
		$hint = "การขอสิทธิ PTM และการช่วยราชการ
			\nให้ทำผ่านระบบ Polis มาโดยช่องปฏิบัติหน้าที่ให้กรอกว่า 'ใช้งานระบบ PTM'
		...
		$regex = '/(ปรับ|การปรับ|ชำระ)(ต่างท้องที่|ต่างสถานี|หน่วยอื่น|ต่าง|ข้ามหน่วย|ข้ามสภ.).{0,10}/i';
		if(preg_match($regex ,$text)){	
			$hint = "กระบวนการปรับต่างท้องที่
			ท้องที่เกิดเหตุออกใบสั่ง->บันทึก ptm-> ผู้ได้รับใบสั่งมาจ่ายต่างท้องที่ -> ท้องที่รับชำระเช็ค ptm (ใช้เมนูค้นหาใบสั่ง หรือเช็คผ่าน next) 
			ถ้ามีข้อมูล รับชำระตามจำนวนใน ptm / ถ้าไม่มีแจ้งท้องที่ออกใบสั่งให้บันทึกใน ptm ก่อน -> เปรียบเทียบปรับ (ลงสมุดเปรียบเทียบ /  ไม่ลงสมุดเปรียบเทียบ) 
			-> ออกใบเสร็จ (แยกเล่ม) -> ส่งเงิน จนท.การเงิน -> จนท.การเงิน บันทึกเงินประจำวัน -> นำเงินเข้าบัญชี ktb corperate online 
			-> ใช้เมนู pay bill ชำระค่าปรับตามใบสั่ง -> ptm ตัดใบสั่งออกจากระบบ -> กรุงไทยส่งเงินหน่วยออกใบสั่ง 
			-> หน่วยออกใบสั่งตรวจสอบยอดชำระ(จาก ptm) และเช็คยอดเงินจาก statement -> แบ่งเงินตามระบบ ครับ";
			break;
		}
